...
float vector_angle(Vector a3, Vector b3)
{
	...
	Vector u = CGAL::cross_product(a3, b3);
	u = v_normalized(u);
	Vector n = CGAL::cross_product(u, a3);
	n = v_normalized(n);
...
int vector_angle_sign(const Vector &a3, const Vector &b3)
{
	...
	if (pl.has_on_positive_side(ap3))
		return 1;
	else
		return -1;
	/*
...
Mesh::Mesh(const Mesh &other)
{
	...
	{
		loaded_filename = new char[strlen(other.loaded_filename) + 1];
		strcpy(loaded_filename, other.loaded_filename);
	}
	else
		loaded_filename = NULL;
	...
	{
		loaded_file_type = new char[strlen(other.loaded_file_type) + 1];
		strcpy(loaded_file_type, other.loaded_file_type);
	}
	else
		loaded_file_type = NULL;
	...
	{
		loaded_vect_filename = new char[strlen(other.loaded_vect_filename) + 1];
		strcpy(loaded_vect_filename, other.loaded_vect_filename);
	}
	else
		loaded_vect_filename = NULL;
...
void Mesh::clear(bool cleanFilenames)
{
	lock();
	p.clear();
	for (int i = 0; i < inter_segments.size(); i++)
		delete inter_segments[i];
	inter_segments.clear();

	tmpFix = 5;
	if ((loaded_filename != NULL) && cleanFilenames)
		delete[] loaded_filename;
	if ((loaded_file_type != NULL) && cleanFilenames)
		delete[] loaded_file_type;
	if ((loaded_vect_filename != NULL) && cleanFilenames)
		delete[] loaded_vect_filename;
	unlock();
}
...
void Mesh::displayInfo()
{
	...
	for (Vertex_iterator v = p.vertices_begin(); v != p.vertices_end(); v++)
		if (isnan(v_norm(v->point() - CGAL::ORIGIN)))
			has_nan = true;
	if (!has_nan)
...
float *Mesh::getBoundingBox()
{
	...
	for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++)
	{
		if (vi->getVisibility() == false)
			continue;
		for (int j = 0; j < 3; j++)
		{
			if (bounding_box[j] > TO_FLOAT(vi->point()[j]))
				bounding_box[j] = TO_FLOAT(vi->point()[j]);
			if (bounding_box[j + 3] < TO_FLOAT(vi->point()[j]))
				bounding_box[j + 3] = TO_FLOAT(vi->point()[j]);
		}
	}
	unlock();
	return bounding_box;
...
Point Mesh::smooth_old_vertex(Vertex_handle v) const
{
	...
	do
	{
		vec = vec + (h->prev()->vertex()->point() - CGAL::ORIGIN) * alpha / degree;
		++h;
...
void Mesh::remeshWithThreshold(float edge_threshold)
{
	...
	while (e != last_e)
	{
		...
		if (h->vertex()->flag[1] && canFlipEdge(h))
		{
			p.flip_edge(h);
			count++;
		}
...
bool Mesh::saveFormat(const char *filename, const char *file_type)
{
	if ((strcmp(file_type, "off") == 0) || (strcmp(file_type, "OFF") == 0))
	{
		...
		{
			os << "OFF" << std::endl
			   << p.size_of_vertices() << ' ' << p.size_of_facets() << " 0" << std::endl;
...
#define DEBUG_LOAD(X)
	...
	if (isPLY)
	{
		...
		for (int i = 0; i < noVertexAttribs; i++)
			newVertexIdx.push_back(iIgnoreIdx);
			...
			if (token.compare("property") == 0)
			{
				...
				if (iCurrentPropType == 1) // vertices
				{
					...
					if (token.compare("x") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexX;
					if (token.compare("y") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexY;
					if (token.compare("z") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexZ;
					if (token.compare("red") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexR;
					if (token.compare("green") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexG;
					if (token.compare("blue") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexB;
					if (token.compare("alpha") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexAlpha;
					if (token.compare("quality") == 0)
						newVertexIdx[iNoVertexAttribs] = MeshLoader::EVertexQuality;
		...
		if (bSupportedFormat)
		{
			...
			for (int i = 0; i < iNoVertices; i++)
			{
				for (int k = 0; k < iNoVertexAttribs; k++)
					newVertex[newVertexIdx[k]] = readTypeFromStream(is, newVertexAttribTypes[k], bBinary);
				builder.addVertex(newVertex);
				DEBUG_LOAD(
					cout << " - vertex ";
					for (int i = 0; i < 6; i++)
...
Mesh::MeshReturnCodes Mesh::loadOBJ(const std::string& filename)
{
	std::string token;
	...
	if (!igl::readOBJ(filename, verts, facets))
		return MeshFileNotFound;
	lock();
	...
	int iNoVertices = verts.size();
	int iNoTriangles = facets.size();

	cout << "* Loading mesh: " << filename << " (" << iNoTriangles << " facets, " << iNoVertices << " vertices)";
	cout << endl;
...
bool Mesh::loadVectorField(const char *filePattern, bool rememberFilename)
{
	...
	cout << "* Loading vectorfield: <" << filePattern << ">";
	cout.flush();
...
void Mesh::convolve(double std_dev_ratio, bool add_to_history, bool print_stats)
{
	...
	for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++)
	{
		...
		if (add_to_history)
		{
			...
			if (vi->values.size() > 1)
				quality_prev = vi->values[vi->values.size() - 2];
...
void Mesh::evolve(float sign)
{
	...
	for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++)
	{
		...
		if (isnan(v_norm(vi->delta)))
			cout << "ISNAN delta" << endl;
		else
		{
...
void Mesh::computeMeshStats()
{
	...
	for (Facet_iterator fi = p.facets_begin(); fi != p.facets_end(); fi++)
	{
		if (fi->halfedge()->vertex()->getVisibility() == false)
			continue;
		tmpArea = fi->area();
		area_avg += tmpArea;
	...
	{
		if (ei->vertex()->getVisibility() == false)
			continue;
		if (ei->prev()->vertex()->getVisibility() == false)
			continue;
	...
	for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++)
	{
		...
		if (vi->laplacian() * vi->normal() < 0.0f)
			tmpNorm = -tmpNorm;
	...
	if (tmpCounter > 1)
		curvature_std_dev = std::sqrt(abs(curvature_std_dev) / (tmpCounter - 1));
...
float Mesh::computeVertexStatistics(Vertex &v, int mode)
{
	...
	{
		if (mode == 0)
			edge_stats = std::min((float)edge_size(h), edge_stats);
		if (mode == 1)
			edge_stats += edge_size(h);
		if (mode == 2)
			edge_stats = std::max((float)edge_size(h), edge_stats);
		no_h++;
	} while (++h != v.vertex_begin());
	if (mode == 1)
...
float Mesh::computeVertexDirectionalGradient(Vertex &v, Vertex &dir_u, QualityComputationMethod qual)
{
	...
	{
		for (int i = 0; i < 3; i++)
			local_deriv += (dir_u.color[i] - v.color[i]);
		local_deriv /= 3;
	}
...
void Mesh::computeVertexCurvature(Vertex &v)
{
	...
	if (curv_comp_method == Curv_Meyer)
	{ //Mark Meyer
		float A_mixed = 0;
	...
	else if (curv_comp_method == Curv_Dong)
	{ //Dong et al.
...
*/
	...
	else if (curv_comp_method == Curv_Approx)
	{ //Approx
	...
	else if (curv_comp_method == Geo_Euclid_Ratio)
	{ // Look in a bigger ring
		...
		patchGeo.push_back(0);
		patchEuclid.push_back(0);
		...
		do
		{
			...
			do
			{
				...
				if (vfind != patchVerts.end())
				{
					...
					if (patchGeo[ifind] > curGeo)
						patchGeo[ifind] = curGeo;
				}
				else
				{
...
#define NUM_NEIGH 10
	...
	colorThreshold = 0.0001;
	distThreshold = 10 * edge_avg;
	...
	if (v_norm(minpt - the_point) < distThreshold)
		return minpt;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void Mesh::splitEdge(Vertex::Halfedge_handle h, int mode)
{ // 1 - middle ; 2-projection
...
{
	if (v->is_trivalent() == false)
		return false;
	if (v->border())
		return false;

	if (v->halfedge()->prev()->opposite()->facet() == v->halfedge()->opposite()->next()->opposite()->facet())
		return false;
	return true;
}
...
bool Mesh::canFlipEdge(Vertex::Halfedge_handle h)
{
	...
	if (v_angle(v_perp_1, v_perp_2) > PI / 180 * 20)
		return false;
	...
	if (valence_mode == 1)
	{
		...
		for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end();)
		{
			...
			if (degree == 4)
			{
				...
				CGAL_For_all(c, d) if (canCollapseEdge(c->opposite()))
				{
...
int Mesh::fixDegeneracy(double collapseRatio, double degenerateAngleDeg)
{
	...
	for (Facet_iterator fi = p.facets_begin(); fi != p.facets_end();)
	{
		...
		do
		{
			...
			if (i == 3)
			{
				...
				if (hf->is_border_edge())
					cout << "forgiven - border edge " << endl;
				return -1;
			}
		...
		for (i = 0; i < 3; i++)
		{
			...
			else if ((tmpAngle > degenerateAngleDeg) && canFlipEdge(edge_h[i]))
			{ //
				flipEdge(edge_h[i]);
				no_ops++;
				break;
...
void Mesh::ensureEdgeSizes(double epsilonMin, double epsilonMax, double collapseRatio, double degenerate_angle_deg, int mode, int max_iters)
{
	...
	while ((((edge_min < epsilonMin)) || (edge_max > epsilonMax)) && (no_ops > 0) && (iters < max_iters))
	{
		...
		{
			Vertex::Halfedge_handle h = bigEdgesQueue.front();
			bigEdgesQueue.pop();
			float avg_mean_curv = max(abs(h->vertex()->mean_curvature()), abs(h->prev()->vertex()->mean_curvature()));
			if ((h->vertex()->getVisibility()) && (h->prev()->vertex()->getVisibility()) && canSplitEdge(h) && (avg_mean_curv > 0.1))
			{
				splitEdge(h);
				no_ops++;
			}
		...
		{ //start a new component
			...
			{ //fill the current component
				...
				do
				{
					...
					if (colorVertices)
						h->vertex()->set_color(color[0], color[1], color[2]);
...
int Mesh::eraseConnectedComponent(Facet_handle &f)
{
	...
	{
		Facet_handle f = facet_erase_queue.front();
		facet_erase_queue.pop();
